...
int process_command(struct command_t *command)
{
	...
	if(strcmp(command->name, "shortdir")==0){
		...
		good_morning(command->args[0],command->args[1]);
	}
		...
		if (strcmp(command->args[0], "-a")==0) {
			...
			kdiffB(command->args[1],command->args[2]);
		}else{
			kdiff(command->args[0],command->args[1]);
		}
		
	}
	else if(strcmp(command->name, "todo")==0){
		if (strcmp(command->args[0], "add")==0) {
			addTodo(command->args[1],command->args[2],command->args[3]);
		}
		else if (strcmp(command->args[0], "del")==0) {
			int i=2;
			while (command->args[i]!=NULL)
			{
				...
				strcat(command->args[1],command->args[i]);
				i++;
			}
			
			delTodo(command->args[1]);
		}
		else if (strcmp(command->args[0], "delproject")==0) {
			delProject(command->args[1]);
		}
		else if (strcmp(command->args[0], "listproject")==0) {
			listProject(command->args[1]);
		}
		else if (strcmp(command->args[0], "listall")==0) {
			sortAllTodos();
		}
	...
	if (pid==0) // child
	{
		...
		pathsToken = strtok(paths, ":");

        while (pathsToken != NULL) {

          char seaCommand[80];
          strcpy(seaCommand, pathsToken);
          strcat(seaCommand, "/");
          strcat(seaCommand, command->args[0]);

          execv(seaCommand, command->args);
